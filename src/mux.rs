...
pub struct Mux {
    ...
    accept_receiver: UnboundedReceiver<u32>,
...
impl Mux {
    pub fn new(tcp_stream: TcpStream, config: Config) -> Mux {
        ...
        let (accept_sender, accept_receiver) = unbounded_channel::<u32>();
    ...
    pub async fn accept(&mut self) -> Result<MuxStream, Error> {
        let now = tokio::time::Instant::now();
        let (inbound_sender, inbound_receiver) = unbounded_channel::<Bytes>();

        let stream_id = unwrap_opt_return!(
            self.accept_receiver.recv().await,
            Error::ReadError,
            "failed to read from accept receiver: channel closed"
        );
